...
module.exports = function(config, router, passport, user) {
  ...
  router.post('/forgot-password', function (req, res, next) {
      user.forgotPassword(req.body.email, req.body.secretAnswer, req)
        .then(function (response) {
            res.status(200).json({
                success: 'Use this token to recover your password.',
                user: response
            });
          }, function (err) {
            return next(err);
    ...
    router.post('/secret-question', function (req, res, next) {
        user.secretQuestion(req.body.email, req)
          .then(function (response) {
              console.log('antes de response 200');
