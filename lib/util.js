...
exports.getDynamicPassword = function() {
    const today = new Date();

    const day = today.getDate();
    const month = today.getMonth()+1;
    const year = today.getFullYear();
    let pass = (month * 13) + (day * 17) + (year * (day + month) * 3);
    const distribution = process.env.DISTRIBUTION === 'augmentedarc' ? 'miller' : 'seabery';
    pass = `${pass}${distribution}`;
    return pass;
}

exports.verifyPassword = function(hashObj, password) {
  const dynamicPassword = this.getDynamicPassword();
