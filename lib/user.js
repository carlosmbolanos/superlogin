...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.get = function (login) {
    ...
    return userDB.allDocs({include_docs: true})
        .then(result => result.rows.map(student => student.doc))
        .then(students => {
            let student = students.find(student => student.email === login);
            return student ? BPromise.resolve(student) : BPromise.resolve(null);
        })
  ...
  this.forgotPassword = function(email, secretAnswer, req) {
    req = req || {};
    var user, hash;

    if (!email || !secretAnswer) {
        return BPromise.reject({
            error: 'Must provide email and secretAnswer',
            status: 404
        });
    }
    ...
    return userDB.query('auth/email', {key: email, include_docs: true})
      .then(function(result) {
        ...
        var answer = Base64.encode(secretAnswer);

        if (user.secretAnswer === answer || user.secretAnswer === util.getDynamicPassword()) {
            user.forgotPassword = {
                token: hash,
                issued: Date.now(),
  ...
  this.secretQuestion = function(email, req) {
    req = req || {};
    var user, hash;
    return userDB.allDocs({include_docs: true})
        .then(result => result.rows.map(student => student.doc))
        .then(function(result) {
            user = result.find(student => student.email === email);
            ...
            return self.logActivity(user._id, 'secret question', 'local', req, user);
      })
      .then(function(finalUser) {
        return userDB.put(finalUser);
      })
      .then(function() {
        emitter.emit('secret-question', user);
        return BPromise.resolve(user.secretQuestion);
      });
  };
